...
var serviceRoleKey = process.env.SUPABASE_SERVICE_ROLE_KEY || process.env.VITE_SUPABASE_SERVICE_ROLE_KEY;
if (!serviceRoleKey) {
...
var sendEmailViaSendGrid = async (toEmail, subject, htmlContent) => {
...
app.post("/api/webhook/stripe", express.raw({ type: "application/json" }), async (req, res) => {
  ...
  switch (event.type) {
    case "checkout.session.completed": {
      ...
      let isNewUser = false;
      let userEmail = session.customer_details?.email || session.customer_email || void 0;
      let userName = session.customer_details?.name || "Booster";
      if (!userId && userEmail) {
        try {
          const { data: existingProfile } = await supabaseAdmin.from("profiles").select("id").eq("email", userEmail).single();
          if (existingProfile) {
            userId = existingProfile.id;
          } else {
            generatedPassword = "Bb" + Math.floor(1e3 + Math.random() * 9e3) + Math.random().toString(36).slice(-4) + "!";
      ...
      if (purchaseType === "subscription") {
        ...
        const tokens = PLAN_TOKENS[planType] || 1e3;
        let subscriptionEndDate = null;
        let stripeSubscriptionId = session.subscription;
        if (stripeSubscriptionId) {
          try {
            const stripeSubscription = await stripe.subscriptions.retrieve(stripeSubscriptionId);
            subscriptionEndDate = new Date(stripeSubscription.current_period_end * 1e3).toISOString();
          } catch (e) {
...
<body style="margin: 0; padding: 0; background-color: #06060f; color: #ffffff; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; -webkit-font-smoothing: antialiased;">
    <table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="background-color: #06060f;">
        <tr>
            <td align="center" style="padding: 40px 0;">
                <table width="100%" max-width="600" cellpadding="0" cellspacing="0" role="presentation" style="max-width: 600px; margin: 0 auto; background-color: #14141e; border: 1px solid rgba(255, 255, 255, 0.08); border-radius: 24px; overflow: hidden;">
                    ...
                    <tr>
                        <td style="padding: 40px 40px 20px 40px; text-align: center;">
                            ...
                            <h1 style="margin: 0; color: #ffffff; font-size: 28px; font-weight: 900; letter-spacing: -0.5px;">Voc\xEA agora \xE9 um Booster! \u{1F680}</h1>
                        </td>
                    </tr>
                    ...
                    <tr>
                        <td style="padding: 20px 40px 40px 40px;">
                            <p style="margin: 0 0 24px 0; color: rgba(255, 255, 255, 0.7); font-size: 16px; line-height: 1.6;">
                                Ol\xE1! Seu pagamento do plano <strong style="color: #ffffff; text-transform: uppercase;">${planType}</strong> foi confirmado! A sua conta j\xE1 foi atualizada e liberada com sucesso.
                            </p>
                            ...
                            <div style="background-color: rgba(139, 92, 246, 0.1); border: 1px solid rgba(139, 92, 246, 0.2); border-radius: 16px; padding: 24px; margin-bottom: 32px;">
                                <h3 style="margin: 0 0 16px 0; color: #a78bfa; font-size: 18px;">\u{1F510} Suas Credenciais de Acesso</h3>
                                <p style="margin: 0 0 12px 0; color: rgba(255, 255, 255, 0.8);">Uma conta foi criada automaticamente para voc\xEA. Use os dados abaixo para entrar:</p>
                                ...
                                <p style="margin: 12px 0 0 0; color: rgba(255, 255, 255, 0.5); font-size: 12px;">Recomendamos alterar sua senha ap\xF3s o primeiro acesso.</p>
                            </div>
                            ` : ""}

                            <p style="margin: 0 0 32px 0; color: rgba(255, 255, 255, 0.7); font-size: 16px; line-height: 1.6;">
                                Prepare-se para criar conte\xFAdos magn\xE9ticos e dominar o algoritmo. Aqui est\xE3o os seus pr\xF3ximos passos para extrair o m\xE1ximo do BlentBoost:
                            </p>
                            ...
                            <div style="background-color: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.08); border-radius: 16px; padding: 24px; margin-bottom: 32px;">
                                <h3 style="margin: 0 0 16px 0; color: #ffffff; font-size: 18px;">\u{1F6E0}\uFE0F Primeiros Passos:</h3>
              ...
              await sendEmailViaSendGrid(user.email, "Bem-vindo(a) ao BlentBoost! \u{1F389}", welcomeHtml);
            }
          } catch (e) {
        ...
        const { data: profileData } = await supabaseAdmin.from("profiles").select("token_balance").eq("id", userId).single();
        const currentBalance = profileData?.token_balance || 0;
        const newBalance = currentBalance + tokens;
        const { error } = await supabaseAdmin.from("profiles").update({ token_balance: newBalance }).eq("id", userId);
        if (error) {
    ...
    case "customer.subscription.deleted": {
      ...
      await supabaseAdmin.from("profiles").update({ subscription_status: "canceled" }).eq("stripe_customer_id", customerId);
    ...
    case "customer.subscription.updated": {
      ...
      const endDate = subscription.current_period_end ? new Date(subscription.current_period_end * 1e3).toISOString() : null;
...
var allowedOrigin = process.env.VITE_APP_URL || "http://localhost:3000";
...
app.post("/api/user/cancel-subscription", authenticateUser, async (req, res) => {
  ...
  try {
    const { data: profileData, error: profileError } = await supabaseAdmin.from("profiles").select("stripe_subscription_id, email, name, subscription_plan").eq("id", userId).single();
    ...
    const cancelHtml = `
      <h2>Solicita\xE7\xE3o de Cancelamento de Assinatura</h2>
      <p><strong>Usu\xE1rio:</strong> ${profileData.name || "N/A"}</p>
      ...
      <p style="color: #888; font-size: 12px;">A assinatura foi configurada para n\xE3o renovar ao fim do ciclo atual.</p>
    `;
    ...
    res.json({ success: true, message: "Assinatura cancelada. Voc\xEA continua com acesso at\xE9 o fim do per\xEDodo atual." });
  } catch (err) {
    console.error("Erro ao cancelar assinatura:", err);
    res.status(500).json({ error: "Ocorreu um erro ao processar o cancelamento." });
  }
});
var consumeTokensSchema = z.object({
  cost: z.number().nonnegative(),
  action: z.string().optional()
});
app.post("/api/consume-tokens", authenticateUser, async (req, res) => {
  ...
  const userId = req.user.id;
  const { data: profile, error: fetchError } = await supabaseAdmin.from("profiles").select("token_balance, is_unlimited").eq("id", userId).single();
  ...
  const { error: updateError } = await supabaseAdmin.from("profiles").update({ token_balance: newBalance }).eq("id", userId);
  if (updateError) {
...
app.post("/api/auth/signup", async (req, res) => {
  ...
  if (!serviceRoleKey) {
    return res.status(500).json({
      error: "Configura\xE7\xE3o incompleta no servidor: SUPABASE_SERVICE_ROLE_KEY n\xE3o encontrada no arquivo .env."
    });
  }
  try {
    ...
    if (authError) {
      ...
      if (authError.message.toLowerCase().includes("bearer token")) {
        return res.status(500).json({
          error: "Erro de configura\xE7\xE3o no servidor: O endpoint de cadastro requer uma chave 'service_role' v\xE1lida ou o servidor precisa ser reiniciado para carregar o arquivo .env."
        });
      }
...
app.post("/api/auth/forgot-password", async (req, res) => {
  ...
  if (!email) {
    return res.status(400).json({ error: "O e-mail \xE9 obrigat\xF3rio." });
  }
  try {
    ...
    if (linkError) {
      ...
      return res.json({ success: true, message: "Se o e-mail existir, um link de recupera\xE7\xE3o ser\xE1 enviado." });
    }
    ...
    const htmlEmail = `
      ...
      <head>
          <meta charset="UTF-8">
          <meta name="viewport" content="width=device-width, initial-scale=1.0">
          <title>Redefini\xE7\xE3o de Senha - BlentBoost</title>
      </head>
      <body style="margin: 0; padding: 0; background-color: #06060f; color: #ffffff; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; -webkit-font-smoothing: antialiased;">
          <table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="background-color: #06060f;">
              <tr>
                      ...
                      <table width="100%" max-width="600" cellpadding="0" cellspacing="0" role="presentation" style="max-width: 600px; margin: 0 auto; background-color: #14141e; border: 1px solid rgba(255, 255, 255, 0.08); border-radius: 24px; overflow: hidden;">
                          ...
                          <tr>
                              <td style="padding: 40px 40px 20px 40px; text-align: center;">
                                  ...
                                  <h1 style="margin: 0; color: #ffffff; font-size: 28px; font-weight: 900; letter-spacing: -0.5px;">Redefini\xE7\xE3o de Senha</h1>
                              </td>
                          </tr>
                          ...
                          <tr>
                              <td style="padding: 20px 40px 40px 40px;">
                                  <p style="margin: 0 0 24px 0; color: rgba(255, 255, 255, 0.7); font-size: 16px; line-height: 1.6;">
                                      Recebemos uma solicita\xE7\xE3o para redefinir a senha da sua conta no <strong style="color: #ffffff;">BlentBoost</strong> associada a este e-mail.
                                  </p>
                                  <p style="margin: 0 0 32px 0; color: rgba(255, 255, 255, 0.7); font-size: 16px; line-height: 1.6;">
                                      Se foi voc\xEA quem fez a solicita\xE7\xE3o, clique no bot\xE3o abaixo para criar uma nova senha:
                                  </p>
                                  ...
                                  <p style="margin: 32px 0 0 0; color: rgba(255, 255, 255, 0.4); font-size: 14px; line-height: 1.5; text-align: center;">
                                      Se voc\xEA n\xE3o solicitou a redefini\xE7\xE3o de senha, pode ignorar este e-mail com seguran\xE7a. Sua senha n\xE3o ser\xE1 alterada at\xE9 que voc\xEA acesse o link acima e crie uma nova.
                                  </p>
                              </td>
                          </tr>
    ...
    if (!sendgridRes.ok) {
      const sendgridErr = await sendgridRes.text();
      console.error("Erro ao enviar e-mail via SendGrid:", sendgridErr);
      return res.status(500).json({ error: "Falha ao enviar e-mail de recupera\xE7\xE3o." });
    }
    res.json({ success: true, message: "E-mail de recupera\xE7\xE3o enviado com sucesso." });
  } catch (err) {
    console.error("Erro na rota de forgot-password:", err.message);
    res.status(500).json({ error: "Ocorreu um erro interno ao solicitar a recupera\xE7\xE3o." });
  }
});
app.post("/api/report-bug", async (req, res) => {
  const { tool, description, userEmail } = req.body;
  if (!description) {
    return res.status(400).json({ error: "A descri\xE7\xE3o do bug \xE9 obrigat\xF3ria." });
  }
  try {
    let userId = null;
    if (userEmail) {
    ...
    const htmlEmail = `
      <h2>Novo Relat\xF3rio de Bug - BlentBoost</h2>
      <p><strong>Usu\xE1rio (Email):</strong> ${userEmail || "N\xE3o informado"}</p>
      <p><strong>Ferramenta/Se\xE7\xE3o:</strong> ${tool || "Geral"}</p>
    ...
    try {
      await sendEmailViaSendGrid(
        "contato@assessoriablent.com",
        `Report de Bug: ${tool || "Geral"} - ${userEmail || "Usu\xE1rio"}`,
        htmlEmail
      );
    } catch (e) {
      console.error("Falha ao enviar e-mail de notifica\xE7\xE3o de bug interno:", e);
    }
    res.json({ success: true, message: "Relat\xF3rio enviado com sucesso." });
  } catch (err) {
    console.error("Erro na rota de report-bug:", err);
    res.status(500).json({ error: "Ocorreu um erro interno ao enviar o relat\xF3rio." });
  }
});
app.get("/api/admin/bug-reports", async (req, res) => {
  try {
    const { data: bugs, error } = await supabaseAdmin.from("bug_reports").select("*").order("created_at", { ascending: false });
    if (error) throw error;
    const userIds = (bugs || []).map((b) => b.user_id).filter(Boolean);
    let profileMap = {};
    if (userIds.length > 0) {
    ...
    const userBugCounts = (bugs || []).reduce((acc, curr) => {
      acc[curr.user_email] = (acc[curr.user_email] || 0) + 1;
      return acc;
    }, {});
    const enrichedBugs = (bugs || []).map((b) => ({
      ...b,
...
app.post("/api/admin/bug-reports/:id/resolve", async (req, res) => {
  try {
...
app.post("/api/admin/bug-reports/:id/thank-you", async (req, res) => {
  try {
    const bugId = req.params.id;
    ...
    if (!bug.user_email || bug.user_email === "unknown@example.com") {
      return res.status(400).json({ error: "Nenhum e-mail v\xE1lido associado a este bug." });
    }
                      ...
                      <table width="100%" max-width="600" cellpadding="0" cellspacing="0" role="presentation" style="max-width: 600px; margin: 0 auto; background-color: #14141e; border: 1px solid rgba(255, 255, 255, 0.08); border-radius: 24px; overflow: hidden;">
                          ...
                          <tr>
                              <td style="padding: 40px 40px 20px 40px; text-align: center;">
                                  ...
                                  <h1 style="margin: 0; color: #ffffff; font-size: 28px; font-weight: 900; letter-spacing: -0.5px;">Obrigado por Ajudar! \u{1F49C}</h1>
                              </td>
                          </tr>
                          ...
                          <tr>
                              <td style="padding: 20px 40px 40px 40px;">
                                  <p style="margin: 0 0 24px 0; color: rgba(255, 255, 255, 0.7); font-size: 16px; line-height: 1.6;">
                                      Ol\xE1! Toda a equipe do <strong style="color: #ffffff;">BlentBoost</strong> quer te agradecer imensamente pelo bug que voc\xEA reportou recentemente na ferramenta de <strong>${bug.tool}</strong>.
                                  </p>
                                  <p style="margin: 0 0 32px 0; color: rgba(255, 255, 255, 0.7); font-size: 16px; line-height: 1.6;">
                                      \xC9 gra\xE7as \xE0 aten\xE7\xE3o impec\xE1vel de usu\xE1rios como voc\xEA que conseguimos manter a plataforma r\xE1pida, est\xE1vel e livre de erros. Seu relato j\xE1 foi analisado e o problema est\xE1 resolvido!
                                  </p>
    ...
    `;
    await sendEmailViaSendGrid(bug.user_email, "Obrigado pelo seu report no BlentBoost! \u{1F680}", htmlEmail);
...
app.post("/api/instagram/publish", authenticateUser, async (req, res) => {
  ...
  try {
    const { data: socialAccount, error: socialError } = await supabaseAdmin.from("user_social_accounts").select("*").eq("user_id", userId).eq("platform", "instagram").single();
    if (socialError || !socialAccount) {
      return res.status(404).json({ error: "Conta do Instagram n\xE3o conectada" });
    }
    ...
    if (!containerResponse.ok || !containerData.id) {
      ...
      return res.status(500).json({ error: "Erro ao preparar m\xEDdia no Instagram", details: containerData });
    }
    ...
    if (!publishResponse.ok || !publishData.id) {
      console.error("Erro ao publicar m\xEDdia:", publishData);
      return res.status(500).json({ error: "Erro ao publicar no Instagram", details: publishData });
    }
    if (postId) {
      await supabaseAdmin.from("planner_posts").update({
        status: "PUBLISHED",
        is_posted: true,
...
app.post("/api/instagram/process-scheduled", async (req, res) => {
  try {
    ...
    const { data: scheduledPosts, error: fetchError } = await supabaseAdmin.from("planner_posts").select("*, profiles(id)").eq("status", "SCHEDULED").lte("scheduled_time", now).eq("is_posted", false);
    ...
    for (const post of scheduledPosts) {
      try {
        const { data: socialAccount } = await supabaseAdmin.from("user_social_accounts").select("*").eq("user_id", post.user_id).eq("platform", "instagram").single();
        if (!socialAccount) {
...
app.get("/api/admin/transactions/:userId", async (req, res) => {
  ...
  const { data, error } = await supabaseAdmin.from("token_transactions").select("*").eq("user_id", userId).order("created_at", { ascending: false }).limit(20);
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Relat\xF3rio de Bug - BlentBoost</title>
</head>
<body style="margin: 0; padding: 0; background-color: #06060f; color: #ffffff; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; -webkit-font-smoothing: antialiased;">
    <table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="background-color: #06060f;">
        <tr>
                    ...
                    <tr>
                        <td style="padding: 20px 40px 40px 40px;">
                            <div style="background-color: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.08); border-radius: 16px; padding: 24px; margin-bottom: 24px;">
                                <p style="margin: 0 0 12px 0; color: rgba(255, 255, 255, 0.7); font-size: 14px;"><strong>Usu\xE1rio:</strong> <span style="color: #ffffff;">${userEmail}</span></p>
                            ...
                            <div style="background-color: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.08); border-radius: 16px; padding: 24px;">
                                <h3 style="margin: 0 0 16px 0; color: #ffffff; font-size: 16px;">Descri\xE7\xE3o do Problema:</h3>
